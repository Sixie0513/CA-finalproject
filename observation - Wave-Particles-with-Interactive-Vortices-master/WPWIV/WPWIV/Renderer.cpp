...
bool Renderer::CreateDepthStencilBuffer(ID3D12Device* device, float Width, float Height)
{
	...
	for (int i = 0; i < FrameBufferCount; i++)
	{
		CD3DX12_HEAP_PROPERTIES heapProperties(D3D12_HEAP_TYPE_DEFAULT); // 將 CD3DX12_HEAP_PROPERTIES 轉換為左值
		CD3DX12_RESOURCE_DESC resourceDesc = CD3DX12_RESOURCE_DESC::Tex2D(DXGI_FORMAT_D32_FLOAT, Width, Height, 1, 0, 1, 0, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL); // 將 CD3DX12_RESOURCE_DESC 轉換為左值

		hr = device->CreateCommittedResource(
			&heapProperties, // 指向堆屬性的指針
			D3D12_HEAP_FLAG_NONE,
			&resourceDesc, // 指向資源描述的指針
...
{
	CD3DX12_RESOURCE_BARRIER transitionBarrier = CD3DX12_RESOURCE_BARRIER::Transition(GetRenderTargetBuffer(frameIndex), D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET);
...
void Renderer::RecordEnd(int frameIndex, ID3D12GraphicsCommandList* commandList)
{
	...
	CD3DX12_RESOURCE_BARRIER transitionBarrier = CD3DX12_RESOURCE_BARRIER::Transition(GetRenderTargetBuffer(frameIndex), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT);
	commandList->ResourceBarrier(1, &transitionBarrier);
}
...
bool Renderer::BindTextureToDescriptorHeap(ID3D12Device* device, ID3D12DescriptorHeap* descriptorHeap, Texture* texture, int slot)
{
	...
	device->CreateShaderResourceView(texture->GetTextureBuffer(), &srvDesc, descriptorHandle);
	return true;
}
...
bool Renderer::BindRenderTextureToRtvDescriptorHeap(ID3D12Device* device, ID3D12DescriptorHeap* descriptorHeap, RenderTexture* texture, int slot)
{
	...
	device->CreateRenderTargetView(texture->GetTextureBuffer(), &rtvDesc, descriptorHandle);
...
bool Renderer::BindRenderTextureToDsvDescriptorHeap(ID3D12Device* device, ID3D12DescriptorHeap* descriptorHeap, RenderTexture* texture, int slot)
{
	...
	D3D12_DEPTH_STENCIL_VIEW_DESC dsvDesc = texture->GetDsvDesc();

	device->CreateDepthStencilView(texture->GetDepthStencilBuffer(), &dsvDesc, descriptorHandle);
	...
	D3D_PRIMITIVE_TOPOLOGY primitiveTypeOverride)//pass in D3D_PRIMITIVE_TOPOLOGY_UNDEFINED to use primitive type of each mesh
{
	...
	for (int i = 0; i < meshCount; i++)
	{
		...
		// 儲存 Vertex Buffer View 和 Index Buffer View
		D3D12_VERTEX_BUFFER_VIEW vertexBufferView = pFrame->GetMeshVec()[i]->GetVertexBufferView();
		D3D12_INDEX_BUFFER_VIEW indexBufferView = pFrame->GetMeshVec()[i]->GetIndexBufferView();
	...
	D3D_PRIMITIVE_TOPOLOGY primitiveTypeOverride)//pass in D3D_PRIMITIVE_TOPOLOGY_UNDEFINED to use primitive type of each mesh
{
	...
	// set the descriptor table to the descriptor heap (parameter 1, as constant buffer root descriptor is parameter index 0)
	commandList->SetGraphicsRootConstantBufferView(UNIFORM_SLOT::CAMERA, pFrame->GetCameraVec()[0]->GetUniformBufferGpuAddress());
	// 儲存 Viewport 和 Scissor Rect
	D3D12_VIEWPORT viewport = pFrame->GetCameraVec()[0]->GetViewport();
	D3D12_RECT scissorRect = pFrame->GetCameraVec()[0]->GetScissorRect();
	...
	for (int i = 0; i < meshCount; i++)
	{
		if (primitiveTypeOverride == D3D_PRIMITIVE_TOPOLOGY_UNDEFINED)
			commandList->IASetPrimitiveTopology(pFrame->GetMeshVec()[i]->GetPrimitiveType());

		// 設置頂點緩衝區和索引緩衝區
		auto vertexBufferView = pFrame->GetMeshVec()[i]->GetVertexBufferView();
		commandList->IASetVertexBuffers(0, 1, &vertexBufferView);

		auto indexBufferView = pFrame->GetMeshVec()[i]->GetIndexBufferView();
		commandList->IASetIndexBuffer(&indexBufferView);

		// 設置常量緩衝區
		commandList->SetGraphicsRootConstantBufferView(UNIFORM_SLOT::OBJECT, pFrame->GetMeshVec()[i]->GetUniformBufferGpuAddress());
