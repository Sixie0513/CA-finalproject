...
bool Texture::CreateTextureBuffer(ID3D12Device* device)
{
	...
	hr = device->CreateCommittedResource(
		...
		D3D12_HEAP_FLAG_NONE, // 沒有標誌
		&textureDesc, // 指向資源描述的指針
		D3D12_RESOURCE_STATE_COPY_DEST, // 從上傳堆中將紋理複製到這裡，所以我們將其起始狀態設為拷貝目標狀態
		nullptr, // 用於渲染目標和深度/模板緩衝區
...
bool Texture::UpdateTextureBuffer(ID3D12Device* device)
{
	...
	CD3DX12_HEAP_PROPERTIES heapProperties(D3D12_HEAP_TYPE_UPLOAD); // 將 CD3DX12_HEAP_PROPERTIES 轉換為左值
	CD3DX12_RESOURCE_DESC resourceDesc = CD3DX12_RESOURCE_DESC::Buffer(textureUploadBufferSize); // 將 CD3DX12_RESOURCE_DESC 轉換為左值

	hr = device->CreateCommittedResource(
		&heapProperties, // 指向堆屬性的指針
		D3D12_HEAP_FLAG_NONE, // 沒有標誌
		&resourceDesc, // 指向資源描述的指針
		D3D12_RESOURCE_STATE_GENERIC_READ, // 將從這個堆複製內容到上面的默認堆
		nullptr,
	...
	CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition( // 將 CD3DX12_RESOURCE_BARRIER 轉換為左值
		textureBuffer, // 資源
		D3D12_RESOURCE_STATE_COPY_DEST, // 之前的狀態：拷貝目標
		D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE // 新狀態：像素著色器資源
	);

	immediateCopyCommandList->ResourceBarrier(1, &barrier); // 執行資源屏障
...
bool RenderTexture::CreateTextureBuffer(ID3D12Device* device)
{
	...
	hr = device->CreateCommittedResource(
		...
		D3D12_HEAP_FLAG_NONE, // 沒有標誌
		&textureDesc, // 指向資源描述的指針
		D3D12_RESOURCE_STATE_RENDER_TARGET, // 資源將用於渲染目標
		&colorOptimizedClearValue, // 渲染目標緩衝區的優化清除值
		IID_PPV_ARGS(&textureBuffer) // 使用 IID_PPV_ARGS 宏將接口指針轉換為 IID 形式
	);
	...
	if (supportDepth)
	{
		...
		CD3DX12_HEAP_PROPERTIES heapProperties(D3D12_HEAP_TYPE_DEFAULT); // 將 CD3DX12_HEAP_PROPERTIES 轉換為左值
		CD3DX12_RESOURCE_DESC resourceDesc = CD3DX12_RESOURCE_DESC::Tex2D( // 將 CD3DX12_RESOURCE_DESC 轉換為左值
		...
		hr = device->CreateCommittedResource(
			...
			D3D12_HEAP_FLAG_NONE, // 沒有標誌
			&resourceDesc, // 指向資源描述的指針
			D3D12_RESOURCE_STATE_DEPTH_WRITE, // 深度緩衝區將用於深度寫入
			&depthOptimizedClearValue, // 深度緩衝區的優化清除值
			IID_PPV_ARGS(&depthStencilBuffer) // 使用 IID_PPV_ARGS 宏將接口指針轉換為 IID 形式
		);
