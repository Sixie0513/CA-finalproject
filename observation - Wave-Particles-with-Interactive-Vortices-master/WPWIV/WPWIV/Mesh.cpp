...
bool Mesh::CreateUniformBuffer(ID3D12Device* device)
{
	...
	CD3DX12_RESOURCE_DESC resourceDesc = CD3DX12_RESOURCE_DESC::Buffer(sizeof(ObjectUniform)); // 將 CD3DX12_RESOURCE_DESC 轉換為左值
...
bool Mesh::CreateVertexBuffer(ID3D12Device* device)
{
	...
	CD3DX12_RESOURCE_DESC resourceDesc = CD3DX12_RESOURCE_DESC::Buffer(vertexBufferSizeInBytes); // 將 CD3DX12_RESOURCE_DESC 轉換為左值
...
bool Mesh::CreateIndexBuffer(ID3D12Device* device)
{
	...
	CD3DX12_HEAP_PROPERTIES heapProperties(D3D12_HEAP_TYPE_DEFAULT); // 將 CD3DX12_HEAP_PROPERTIES 轉換為左值
	CD3DX12_RESOURCE_DESC resourceDesc = CD3DX12_RESOURCE_DESC::Buffer(indexBufferSizeInBytes); // 將 CD3DX12_RESOURCE_DESC 轉換為左值

	hr = device->CreateCommittedResource(
		&heapProperties, // 指向堆屬性的指針
		D3D12_HEAP_FLAG_NONE, // 沒有標誌
		&resourceDesc, // 指向資源描述的指針
		D3D12_RESOURCE_STATE_COPY_DEST, // 起始狀態為複製目的地狀態
		nullptr, // 對於這類資源，優化的清除值必須為空
		IID_PPV_ARGS(&gpuIndexBuffer) // 使用 IID_PPV_ARGS 宏將接口指針轉換為 IID 形式
	);
...
bool Mesh::UpdateVertexBuffer(ID3D12Device* device)
{
	...
	hr = device->CreateCommittedResource(
		...
		D3D12_HEAP_FLAG_NONE, // 沒有標誌
		&resourceDesc, // 指向資源描述的指針
		D3D12_RESOURCE_STATE_GENERIC_READ, // 資源將被 GPU 讀取，並將其內容複製到默認堆
		nullptr, // 對於這類資源，優化的清除值必須為空
		IID_PPV_ARGS(&immediateCopyBuffer) // 使用 IID_PPV_ARGS 宏將接口指針轉換為 IID 形式
	);
	...
	CD3DX12_RESOURCE_BARRIER transitionBarrier = CD3DX12_RESOURCE_BARRIER::Transition(gpuIndexBuffer, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
	// transition the vertex buffer data from copy destination state to vertex buffer state
	immediateCopyCommandList->ResourceBarrier(1, &transitionBarrier);
...
bool Mesh::UpdateIndexBuffer(ID3D12Device* device)
{
	...
	CD3DX12_HEAP_PROPERTIES heapProperties(D3D12_HEAP_TYPE_UPLOAD); // upload heap
	CD3DX12_RESOURCE_DESC resourceDesc = CD3DX12_RESOURCE_DESC::Buffer(indexBufferSizeInBytes); // 將 CD3DX12_RESOURCE_DESC 轉換為左值
